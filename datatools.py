...
"""
    Budgie ShowData - Shows Output of vnstat on the desktop
    Authors: TJ Thornsberry, Jacob Vlijm
    Copyright (C) 2020  Ubuntu Budgie Developers
...
def get_days_in_cycle():
    now = datetime.now()
    delta1 = datetime(now.year, now.month, int(vnstat_config["MonthRotate"]))
    delta2 = delta1 - now
    if delta2.days < 0:
        delta1 = datetime(now.year, now.month+1, int(vnstat_config["MonthRotate"]))
