...
class BudgieShowDataSettings(Gtk.Grid):
    def __init__(self, setting):
        ...
        self.box_cycle = Gtk.Box()
        self.box_shadow = Gtk.Box()
        self.box_datacap = Gtk.Box()
        ...
        self.cycle_color = Gtk.Button()
        self.shadow_color = Gtk.Button()
        self.apply = Gtk.Button("OK")
        self.datacap_button = Gtk.Button("OK")
        # -Checkboxes
        ...
        self.display_cycle = Gtk.CheckButton(" Show Billing Cycle")
        ...
        self.datacap_combo = self.setup_datacap_type()
        # -Labels
        interface_label = Gtk.Label(" Interface:")
        display_label = Gtk.Label(" Display Type:")
        datacap_label = Gtk.Label(" Datacap:")
        ...
        cycle_label = Gtk.Label("    Set color for Cycle")
        ...
        # -Spinboxes
        adj1 = Gtk.Adjustment(1.0, 1.0, 1000.0, 1.0, 50.0, 0.0)
        self.datacap_spinbox = Gtk.SpinButton()
        self.datacap_spinbox.set_adjustment(adj1)
        self.datacap_spinbox.set_numeric(True)
        self.datacap_spinbox.set_wrap(True)
        self.datacap_spinbox.set_value(int(clt.config["GENERAL"]["datacap_num"]))
        # attach to grid
        ...
        self.box_cycle.pack_start(self.cycle_color, False, False, 0)
        self.box_cycle.pack_start(cycle_label, False, False, 0)
        self.box_shadow.pack_start(self.shadow_color, False, False, 0)
        self.box_shadow.pack_start(shadow_label, False, False, 0)
        self.box_datacap.pack_start(datacap_label, False, False, 0)
        self.box_datacap.pack_start(self.datacap_spinbox, False, False, 0)
        self.box_datacap.pack_start(self.datacap_combo, False, False, 0)
        self.box_datacap.pack_end(self.datacap_button, False, False, 0)
        ...
        self.attach(self.display_cycle, 1, 6, 1, 1)
        self.attach(self.display_shadow, 1, 7, 1, 1)
        self.attach(self.box_yesterday, 1, 8, 1, 1)
        self.attach(self.box_today, 1, 9, 1, 1)
        self.attach(self.box_week, 1, 10, 1, 1)
        self.attach(self.box_month, 1, 11, 1, 1)
        self.attach(self.box_cycle, 1, 12, 1, 1)
        self.attach(self.box_shadow, 1, 13, 1, 1)
        self.attach(spacer1, 1, 14, 1, 1)
        self.attach(self.box_datacap, 1, 15, 1, 1)
        self.attach(spacer2, 1, 16, 1, 1)
        self.attach(self.set_pos_button, 1, 17, 1, 1)
        self.attach(self.box_position, 1, 18, 1, 1)
        self.attach(spacer3, 1, 19, 1, 1)
        self.attach(footnote_2, 1, 20, 1, 1)
        self.attach(footnote_1, 1, 21, 1, 1)
        ...
        self.display_cycle.set_active(self.mute_cycle)
        ...
        self.set_cycle_state(self.mute_cycle)
        ...
        self.display_cycle.connect("toggled", self.toggle_show, "cycle")
        ...
        self.datacap_button.connect("clicked", self.update_datacap)
    ...
    @staticmethod
    def setup_datacap_type():
        datacap_options = Gtk.ListStore(str)
        for item in ["KB", "MB", "GB", "TB"]:
            datacap_options.append([item])
        display_combo = Gtk.ComboBox.new_with_model(datacap_options)
        renderer_text = Gtk.CellRendererText()
        renderer_text.set_property("xalign", 0.51)
        display_combo.pack_start(renderer_text, True)
        display_combo.add_attribute(renderer_text, "text", 0)
        display_combo.set_active(int(clt.config["GENERAL"]["datacap_type"]))
        return display_combo

    def setup_color_buttons(self):
        color_order = ["yesterday", "today", "week", "month", "cycle", "shadow"]
        color_buttons = [self.y_color, self.t_color, self.w_color, self.m_color, self.cycle_color, self.shadow_color]
    ...
    def set_cycle_state(self, val=None):
        self.mute_cycle = not clt.config["MUTE"].getboolean("cycle")
        val = val if val else self.mute_cycle
        for item in [self.box_cycle, self.cycle_color]:
            item.set_sensitive(val)
    ...
    def update_color(self, *args):
        ...
        self.color_button(self.cycle_color, cycle_color)
